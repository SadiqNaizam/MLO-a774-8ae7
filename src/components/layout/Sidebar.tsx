...
const NavLink: React.FC<NavLinkProps> = ({ item, isSubItem = false }) => {
  const Icon = item.icon;
  return (
    <Button
      variant="ghost"
      className={cn(
        "w-full justify-start text-sm font-normal hover:bg-primary/10 hover:text-primary text-sidebar-foreground",
      ...
      asChild={!item.subItems && !!item.href}
    >
      {item.href && !item.subItems ? (
        <a href={item.href} className="flex items-center w-full">
          <Icon className={cn("mr-2 h-4 w-4 shrink-0", item.isActive ? "text-primary-foreground": "text-sidebar-foreground/70 group-hover:text-primary")} />
          {item.label}
        </a>
      ) : (
        <div className="flex items-center w-full">
          <Icon className={cn("mr-2 h-4 w-4 shrink-0", item.isActive ? "text-primary-foreground": "text-sidebar-foreground/70 group-hover:text-primary")} />
          {item.label}
          {item.subItems && <ChevronDown className="ml-auto h-4 w-4 shrink-0 transition-transform duration-200 group-data-[state=open]:rotate-180" />}
        </div>
      )}
    </Button>
  );
...
const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  const [openSections, setOpenSections] = React.useState<string[]>(['dashboards', 'minimal']);

  const renderNavItems = (items: NavItem[], isSubItemLevel = false) => {
      ...
      if (item.subItems) {
        return (
          <AccordionItem value={item.id} key={item.id} className="border-none">
            <AccordionTrigger 
              className={cn(
                "hover:no-underline p-0 rounded-md",
                isSubItemLevel ? "" : "", // No specific style change for top-level trigger from NavLink
                item.isActive && "bg-primary text-primary-foreground hover:bg-primary/90 hover:text-primary-foreground"
              )}
              asChild
            >
            ...
            <AccordionContent className="pb-0 pl-4 pr-0 pt-1 space-y-1">
  ...
  return (
    <aside className={cn("w-64 bg-sidebar text-sidebar-foreground fixed top-0 left-0 h-screen flex flex-col z-50", className)}>
      <div className="h-16 flex items-center px-4 border-b border-sidebar-foreground/10 shrink-0">
        ...
        <div>
          {uiKitNavigationItems.map(category => (
            <div key={category.id} className="mb-4">
              ...
              {category.subItems && (
                 <Accordion type="multiple" defaultValue={category.subItems.length > 0 ? [category.subItems[0].id] : []} className="w-full space-y-1">
                  {renderNavItems(category.subItems, false)}
                </Accordion>
              )}
            </div>
          ))}
      ...
      {/* Possible footer for user actions, if needed later */}
    </aside>
  );
};
